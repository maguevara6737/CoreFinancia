...
#-----------------------------------------------------------------------------------------
presenta error en este bloque de código:
        # Vincular financiaciones pendientes
        Financiacion.objects.filter(
            numero_documento=obj.cliente_id,
            cliente__isnull=True
        ).update(cliente=obj)
        
  File "/root/CoreFinancia/corefinancia_pedro/appfinancia/admin.py", line 368, in ClientesAdmin
    numero_documento=obj.cliente_id,
                     ^^^
...
class ClientesAdmin(admin.ModelAdmin):
    ...
    def save_model(self, request, obj, form, change):
        ...
        if obj.cliente_id:
            Financiacion.objects.filter(
                numero_documento=obj.cliente_id_id,
                cliente__isnull=True
            ).update(cliente=obj)
